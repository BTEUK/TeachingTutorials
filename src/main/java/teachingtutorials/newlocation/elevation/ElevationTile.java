...
import teachingtutorials.newlocation.surfacebuilder.APIService;
...
public class ElevationTile {
    ...
    public ElevationTile(String name, int x, int y, int zoom) {
        ...
        bDownloaded = apiService.downloadImage("https://s3.amazonaws.com/elevation-tiles-prod/terrarium/" + this.zoom + "/" + x + "/" + y + ".png", x, y, zoom);

        //Only attempts to read the file if the file was downloaded successfully
        if (bDownloaded)
        {
    ...
    public double getHeight(int pixelX, int pixelZ) {
        ...
        {
            //Bukkit.getConsoleSender().sendMessage(ChatColor.RED +"Error getting height from pixel: "+e.getMessage());
            return -30;
        }
    }
}
