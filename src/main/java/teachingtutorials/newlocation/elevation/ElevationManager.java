...
public class ElevationManager {
    ...
    public ElevationManager(GeographicProjection projection) {
    ...
    public int[][] getHeights(int xmin, int xmax, int zmin, int zmax) throws OutOfProjectionBoundsException
    {
        double[] lons = null;
        double[] lats = null;
        double[][] elevs = null;
        ...
        {
            Bukkit.getConsoleSender().sendMessage(ChatColor.RED +"Invalid coordinate 0.");
            return new int[16][16];
        }
        else if (lons[0] == lons[1] | lats[0] == lats[1])
        {
            Bukkit.getConsoleSender().sendMessage(ChatColor.RED +"Invalid coordinates 0=1");
            return new int[16][16];
        }
        else if (lons[lons.length-2] == lons[lons.length-1] || lats[lats.length-2] == lats[lats.length-1])
        {
            Bukkit.getConsoleSender().sendMessage(ChatColor.RED +"Invalid coordinates Max=Max-1");
            return new int[16][16];
        }
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                ...
                if (lon < lons[0] || lon > lons[lons.length - 1] || lat > lats[0] || lat < lats[lats.length - 1]) {
                    Bukkit.getConsoleSender().sendMessage(ChatColor.AQUA +"Invalid coordinates, outside range.");
                    return new int[16][16];
                }
